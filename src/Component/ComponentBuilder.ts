...
export default class ComponentBuilder<T extends MessageActionRow | ModalActionRow = MessageActionRow | ModalActionRow> {
    ...
    authorID?: string;
    rowMax: RowMax = 5;
    constructor(authorID?: string, rowMax?: RowMax) {
        if (rowMax) {
            this.rowMax = rowMax;
        }
        if (authorID) {
    ...
    addInteractionButton(options: AddInteractionButtonOptions): this {
        this.addComponent(new Button(options.style, options.customID, options.authorID)["load"](options.style, options.customID, options.authorID, options.label, options.emoji, options.disabled));
        return this;
    }
    ...
    addSelectMenu(options: AddSelectMenuOptions): this {
        this.addComponent(new SelectMenu(options.type, options.customID, options.authorID)["load"](options.customID, options.options, options.placeholder, options.minValues, options.maxValues, options.disabled));
        return this;
    }
    ...
    addTextInput(options: AddTextMenuOptions): this {
        this.addComponent(new TextInput(options.style, options.label, options.customID, options.authorID)["load"](options.style, options.label, options.customID, options.placeholder, options.value, options.minLength, options.maxLength, options.required));
        return this;
    }
    ...
    addURLButton(options: AddURLButtonOptions): this {
        this.addComponent(new Button(ButtonStyles.LINK, options.url, options.authorID)["load"](ButtonStyles.LINK, options.url, options.authorID,options.label, options.emoji, options.disabled));
        return this;
    }
...
/** A developer-defined identifier for the button, max 100 characters. */
    ...
    /** Author identifier for the button in case of a private accessed button */
    authorID: string;
}
...
/** A developer-defined identifier for the input, max 100 characters. */
    ...
    /** Author identifier for the input in case of a private accessed input */
    authorID: string;
}
